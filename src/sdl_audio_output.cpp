...
bool SDLAudioOutput::open(RingBuffer &ringBuf, int sampleRate, int channels)
{
    ...
    if (devId_ <= 0) { ringBuf_ = nullptr; return false; }
    return true;
}
...
{
    if (devId_ > 0) { SDL_PauseAudioDevice(devId_, 0); playing_.store(true); }
}

void SDLAudioOutput::pause()
{
    if (devId_ > 0) { SDL_PauseAudioDevice(devId_, 1); playing_.store(false); }
}
...
float SDLAudioOutput::volume() const { return volume_.load(); }
...
void SDLAudioOutput::audioCallback(void *userdata, uint8_t *stream, int len)
{
    ...
    if (vol < 1.0f) {
        ...
        for (size_t i = 0; i < count; ++i) s[i] *= vol;
    }
}
